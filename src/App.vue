<template>
  <div id="app" class="contain">
    <game-state-start v-if="uiState === 'start'">
      <p
        v-for="(option, i) in characterChoices"
        :key="i"
      ...
      <button @click="pickACharacter">Pick a character</button>
    </game-state-start>
    <section v-else-if="uiState ==='chosenCharacter'">
...
import GameFinistState from './components/GameFinistState.vue';
...
export default {
  ...
  methods: {
    pickACharacter() {
      ...
      this.$store.commit("updateUiState", "chosenCharacter");
    },
...
body {
  ...
  background: url("./assets/illustrations/background.svg") no-repeat center
      center scroll,
    #29abe2;
}
